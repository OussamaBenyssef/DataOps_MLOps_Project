...
from pyspark.sql.functions import col, avg, stddev, lag, when, lit, round as spark_round
...
def calculate_sma(df: DataFrame, price_col: str = "close", period: int = 20, partition_cols: list = None) -> DataFrame:
    """
    ...
    window_spec = Window.partitionBy(*partition_cols).orderBy("timestamp").rowsBetween(-(period - 1), 0)

    result_df = df.withColumn(f"sma_{period}", spark_round(avg(col(price_col)).over(window_spec), 8))

    logger.info(f"✅ SMA_{period} calculated")
    return result_df


def calculate_ema(df: DataFrame, price_col: str = "close", period: int = 12, partition_cols: list = None) -> DataFrame:
    """
    ...
    result_df = df.withColumn(ema_col, spark_round(avg(col(price_col)).over(window_spec), 8))
...
def calculate_rsi(
    df: DataFrame, price_col: str = "close", period: int = None, partition_cols: list = None
) -> DataFrame:
    """
    ...
    window_avg = Window.partitionBy(*partition_cols).orderBy("timestamp").rowsBetween(-(period - 1), 0)

    # Calculate price changes (prefixed to avoid collision with pipeline's price_change)
    df_with_change = df.withColumn("_rsi_price_change", col(price_col) - lag(col(price_col), 1).over(window_spec))

    # Separate gains and losses
    df_with_gains_losses = df_with_change.withColumn(
        "_rsi_gain", when(col("_rsi_price_change") > 0, col("_rsi_price_change")).otherwise(lit(0))
    ).withColumn("_rsi_loss", when(col("_rsi_price_change") < 0, -col("_rsi_price_change")).otherwise(lit(0)))

    # Calculate average gain and loss
    df_with_avg = df_with_gains_losses.withColumn("_rsi_avg_gain", avg(col("_rsi_gain")).over(window_avg)).withColumn(
        "_rsi_avg_loss", avg(col("_rsi_loss")).over(window_avg)
    )

    # Calculate RSI
    result_df = (
        df_with_avg.withColumn(
            "_rsi_rs", when(col("_rsi_avg_loss") != 0, col("_rsi_avg_gain") / col("_rsi_avg_loss")).otherwise(lit(100))
        )
        .withColumn(f"rsi_{period}", spark_round(100 - (100 / (1 + col("_rsi_rs"))), 2))
...
def calculate_macd(
    ...
    df_with_macd = df_with_emas.withColumn(
        "macd", spark_round(col(f"ema_{fast_period}") - col(f"ema_{slow_period}"), 8)
    )

    # Calculate signal line (EMA of MACD)
    window_spec = Window.partitionBy(*partition_cols).orderBy("timestamp").rowsBetween(-(signal_period - 1), 0)

    df_with_signal = df_with_macd.withColumn("macd_signal", spark_round(avg(col("macd")).over(window_spec), 8))

    # Calculate MACD histogram
    result_df = df_with_signal.withColumn("macd_histogram", spark_round(col("macd") - col("macd_signal"), 8)).drop(
        f"ema_{fast_period}", f"ema_{slow_period}"
    )
...
def calculate_bollinger_bands(
    ...
    df_with_sma = df.withColumn("bollinger_middle", spark_round(avg(col(price_col)).over(window_spec), 8))

    # Calculate standard deviation
    df_with_std = df_with_sma.withColumn("std_dev", stddev(col(price_col)).over(window_spec))

    # Calculate upper and lower bands
    result_df = (
        df_with_std.withColumn(
            "bollinger_upper", spark_round(col("bollinger_middle") + (col("std_dev") * std_multiplier), 8)
        )
        .withColumn("bollinger_lower", spark_round(col("bollinger_middle") - (col("std_dev") * std_multiplier), 8))
        .drop("std_dev")
    )
...
def calculate_all_indicators(df: DataFrame, partition_cols: list = None) -> DataFrame:
    """
