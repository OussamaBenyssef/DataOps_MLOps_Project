...
from pyspark.sql.functions import (
    ...
    unix_timestamp, current_timestamp, expr
)
import logging
...
def add_metadata_columns(df: DataFrame) -> DataFrame:
    ...
    enriched_df = (df
        ...
        .withColumn("processing_timestamp", unix_timestamp(current_timestamp()))
    )
...
def detect_price_spikes(
    ...
    result_df = df.withColumn(
        "is_price_spike",
        when(
            (col("price_change_percent") > threshold_percent) |
            (col("price_change_percent") < -threshold_percent),
            lit(True)
        ).otherwise(lit(False))
    )
...
def calculate_volume_metrics(df: DataFrame) -> DataFrame:
    ...
    result_df = (df
        .withColumn(
            "volume_usd",
            spark_round(col("volume") * col("close"), 2)
        )
        .withColumn(
            "avg_trade_size",
            when(col("trades_count") > 0,
                 spark_round(col("volume") / col("trades_count"), 8)
            ).otherwise(lit(0))
        )
    )
