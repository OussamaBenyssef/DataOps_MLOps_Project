...
def process_message(topic, message_value):
    ...
    try:
        if topic == "raw_trades":
            ...
            logger.info(f"Valid trade received for {validated_data.symbol}: {validated_data.quantity} @ {validated_data.price}")
            ...
            logger.info(f"Valid kline received for {validated_data.symbol}: Open={validated_data.kline.open_price}, Close={validated_data.kline.close_price}")
        else:
